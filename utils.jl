...
function mgcg_CUDA(mg_struct_CUDA;nx=64,ny=64,n_level=3,v1=5,v2=5,v3=5, ω=1.0, ω_richardson=2/1000, max_cg_iter=(nx+1)^2, max_mg_iterations=1,iter_algo_num=3, precond=true,dynamic_richardson_ω=false, show_error=false)
    ...
    for k in 1:max_cg_iter
        ...
        if norm(mg_struct_CUDA.r_new_CUDA[1]) < 1e-6 * init_rms && L2_error <= 2 * disc_error
            break
        end
...
function mgcg_CUDA_SpMV(mg_struct_CUDA;nx=64,ny=64,n_level=3,v1=5,v2=5,v3=5, ω=1.0, ω_richardson=2/1000, max_cg_iter=(nx+1)^2, max_mg_iterations=1,iter_algo_num=3, precond=true,dynamic_richardson_ω=false, show_error=false)
    if length(mg_struct_CUDA.lnx_mg) == 0 || nx != mg_struct_CUDA.lnx_mg[1]
        clear_mg_struct_CUDA(mg_struct_CUDA)
        initialize_mg_struct_CUDA(mg_struct_CUDA, nx, ny, n_level)
    end
